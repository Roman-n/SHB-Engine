...
public:
	...
	{
		int	_base = iFloor((base + 200) / 0.2f);
		clamp(_base, 0, 4095);
		y_base = _base;
		float _error = base - r_ybase( );
		int	_height = iCeil((height + _error) / 0.1f);
		clamp(_height, 0, 255);
		y_height = _height;
	}
	...
	u32				w_qclr		(float v, u32 range)	{	int _v = iFloor(v * float(range)); clamp(_v,0, int(range)); return _v; };
