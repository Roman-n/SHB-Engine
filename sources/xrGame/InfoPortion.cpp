...
void CInfoPortion::load_shared	(const char*)
{
	...
	{
		const char* article_str_id = pXML->Read(pNode, "article", i, NULL);
	...
	for(i=0; i<articles_num; ++i)
	{
		const char* article_str_id = pXML->Read(pNode, "article_disable", i, NULL);
		THROW(article_str_id);
	...
	{
		const char* task_str_id = pXML->Read(pNode, "task", i, NULL);
		THROW(task_str_id);
