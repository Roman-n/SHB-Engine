...
CWeaponMagazined::CWeaponMagazined(const char* name, ESoundTypes eSoundType) : CWeapon(name)
{
...
//	VERIFY2(io && (ii == io->inventory( ).ActiveItem( )), make_string("item[%s], parent[%s]", *cName( ), H_Parent( ) ? *H_Parent( )->cName( ) : "no_parent"));
...
bool CWeaponMagazined::Action(int cmd, u32 flags)
{
