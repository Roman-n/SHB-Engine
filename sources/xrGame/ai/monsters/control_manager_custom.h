...
public:
	...
	void		ta_pointbreak			();
	void		ta_fill_data			(SAnimationTripleData &data, const char* s1, const char* s2, const char* s3, bool execute_once, bool skip_prep, u32 capture_type = ControlCom::eCaptureDir | ControlCom::eCapturePath | ControlCom::eCaptureMovement);
	bool		ta_is_active			();
	...
	void		load_jump_data			(const char* s1, const char* s2, const char* s3, const char* s4, u32 vel_mask_prepare, u32 vel_mask_ground, u32 flags);
	
	void		script_jump				(const Fvector &position, float factor);

	//-------------------------------------------------------------------------------
	// Rotation Jump
	void		add_rotation_jump_data	(const char* left1, const char* left2, const char* right1, const char* right2, float angle, u32 flags = 0);
	void		add_melee_jump_data		(const char* left, const char* right);

	//-------------------------------------------------------------------------------
	// Threaten Animation
	void		set_threaten_data		(const char* anim, float time) {m_threaten_anim = anim; m_threaten_time = time;}

	void		critical_wound			(const char* anim);
...
private:
	...
	void		fill_rotation_data		(SControlRotationJumpData &data, const char* left1, const char* left2, const char* right1, const char* right2, float angle, u32 flags);
};
